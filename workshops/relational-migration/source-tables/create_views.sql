...
CREATE OR REPLACE VIEW vCustOrdersUnion_AP1 AS
    SELECT
      cust_id,
      ord_id,
      rep_id,
    ...
    FROM
       vCustOrdersUnion
    WHERE
      ((vCustOrdersUnion.region = 'North')
        AND
      (vCustOrdersUnion.ord_date > '2023-08-01'));

SELECT 'created view vCustOrdersUnion_AP1' as '';

CREATE OR REPLACE VIEW vCustNorth AS
    SELECT * FROM Customers WHERE region = 'North';

SELECT 'created view vCustNorth' as '';

CREATE OR REPLACE VIEW vReps AS
    SELECT * FROM Reps WHERE rep_name LIKE 'L%';

SELECT 'created view vReps' as '';

...
const randomString = (len) => {
    const myUUID =  randomUUID().replace(/-/g, '').slice(-len);
    return myUUID;
}
...
const pseudoRandomElement = (arr, a) => {
    let seed = a;
    if (seed === undefined) {
        seed = 0;
    }
    ...
    return function() {
        ...
        const randNum = (((t ^ t >>> 14) >>> 0) / 4294967296);
        if(Array.isArray(arr)) {
            return arr[Math.floor( randNum * arr.length)];
        } else {
            return Math.floor(randNum * arr);
        }
    }
}

const payloadData = (kb) => {
    let payload;
    ...
    if(Math.floor(kb) === kb) {
        payload = oneKb.repeat(kb);
    } else {
        if(kb > 1) {
            payload = oneKb.repeat(Math.floor(kb));
        }

        let extraChars = Math.floor((kb - Math.floor(kb)) * 1024);
        payload += oneKb.slice(0,extraChars);
    }
...
export {randomString, md5, randomElement, pseudoRandomElement, payloadData};

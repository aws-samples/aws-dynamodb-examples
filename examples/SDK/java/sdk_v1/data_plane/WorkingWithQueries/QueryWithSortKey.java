...
public class QueryWithSortKey {
    ...
    public static void main(String[] args){
        ...
        try{
            ...
            Key fromKey = Key.builder().partitionValue("jim.bob@somewhere.com").sortValue("2021-01-01").build();
            Key toKey = Key.builder().partitionValue("jim.bob@somewhere.com").sortValue("2021-06-01").build();


            // Query condition between January and June
            QueryConditional queryConditional = QueryConditional.sortBetween(fromKey, toKey);

            // Get all records matching dates
            PageIterable<Customer> customers =
                    table.query(r -> r.queryConditional(queryConditional));

            customers.stream()
                    .forEach(p -> p.items().forEach(item -> System.out.println(item.getName())));
